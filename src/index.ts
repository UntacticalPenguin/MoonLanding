...
function run_shuttle(height_value, velocity_value, fuel_value, brake_button){
  ...
  var intervalID = setInterval(function(){
    ...
    if (height_hard == false){height_value.textContent = "" + Math.round(show_height*0.1*100)/100 + " m";}
    else{ height_value.textContent = "????????"}
    if (velocity_hard == false){velocity_value.textContent = "" + Math.round(show_velocity*100)/100 + " m/s2";}
    else{
      velocity_value.textContent = "????????"
    }
    if (fuel_hard == false){fuel_value.textContent = "" + shuttle.fuel;}
    else{ fuel_value.textContent = "????????"}
...
class Shuttle{
  ...
  brake: number;
  status: string;
  moon_gravitation: number;
  static instance: Shuttle;
  private static _instance: Shuttle;
  constructor(){
    this.fuel = 10000
    this.height = 500000;
    this.velocity = 1000;
  ...
  public run_calc(brake_status:Boolean){
    this.status = "running"
    let new_height = 0;
    let new_velocity = 0;
